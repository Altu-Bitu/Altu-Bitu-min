...
int calcLostRupee(matrix &cave, int n)
{
    ...
    matrix lost_rupees(n, vector<int> (n, 1250));   // 해당 정점에 도착할 때까지 총 잃은 루피 수
    queue<pair<int, int>> q;    // <동굴의 x좌표, y좌표>
    ...
    while (!q.empty())
    {
        ...
        {
            // 다음으로 이동할 정점의 좌표
            int next_x = v.first + dx[i];
            int next_y = v.second + dy[i];
            ...
            int new_losts = lost_rupees[v.first][v.second] + cave[next_x][next_y];
